...
class ExpenseSummaryStats(APIView):
    ...
    def get(self, request):
        ...
        expenses = Expense.objects.filter(owner=request.user, date__gte=ayear_ago, date__lte=todays_date) #lte ~ <=
        
        final = {}
        categories =  list(set(map(self.get_category, expenses)))   #Returns distinct list of categories from expenses
...
class IncomeSourcesSummaryStats(APIView):
    ...
    def get(self, request):
        todays_date = datetime.date.today()
        ayear_ago = todays_date - datetime.timedelta(days=30*12)
        income = Income.objects.filter(owner=request.user, date__gte=ayear_ago, date__lte=todays_date) #lte ~ <=
        
        final = {}
        sources =  list(set(map(self.get_source, income)))   #Returns distinct list of categories from expenses
